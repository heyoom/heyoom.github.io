...
2. **선택적 발행**: `published: true` 플래그로 제어
...
**발행 타입**:
- `published: true` (기본) → `content/posts/` (블로그 포스트, 목록 표시)
- `published: true` + `type: page` → `content/` (독립 페이지, 메뉴/링크로만 접근)
...
- **기본 (증분 sync)**: 변경된 파일만 변환 (mtime 기반)
  - 빠름 (수백 개 파일 중 변경된 것만)
  - `published: false`로 변경된 파일은 자동 삭제
  - 일상적인 작업에 사용
...
# 최근 변경 파일 (vault)
find -L obsidian-vault -name "*.md" -mtime -7 -exec grep -l "published: true" {} \;
